...
def model_process(gray_frame, model, noise_thres, max_area):
    """
    ...
    for c in contours:
        #   去除小面积变化噪声
        if cv.contourArea(c) < noise_thres * 2 or cv.contourArea(c) >= max_area:
            continue
        #boundingRect找到最小矩形包住宅c
    ...
    moderate_contours = []
    for c in contours:
        if cv.contourArea(c) > noise_thres * 8 and cv.contourArea(c) < max_area:
...
def is_big_motion(queue_contours, area_thres, slope_thres, num_thres):
    ...
    如果关联比例relevance与关联时间relevance_time取得好，可以不判断是否要处理，而是都进行前后帧关联处理，这样可以减少一些错误噪声，但是会降低对运动的敏感性。
...
def get_better_contours(queue_contours,  area_thres, slope_thres, num_thres, relevance):
    ...
    ，因此，如果关联帧中某区域一直运动，那么可以认为该区域属于运动区域，relevance越大，运动区域的识别准确率越高
...
def add_log(logs, frame, size):
    ...
    for log in logs:
        print(height,log)
        height += per_height
        cv.putText(frame, log, (width, height), cv.FONT_HERSHEY_PLAIN, font_size, (0, 255, 0), 2)
...
def motion_detect(src, area_thres, slope_thres, num_thres, relevance, relevance_time, noise_proportion, show_frame, show_log, output, skip_time):
    ...
    size = (min(1280,size[0]),min(720,size[1]))
    print((int(cap.get(cv.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv.CAP_PROP_FRAME_HEIGHT))), '-->', size)
    max_area = size[0] * size[1] * 0.8
    ...
    print('noise_thres', noise_thres)
    queue_max_len = fps * relevance_time // 2 * 2 + 1
    print('max_len',queue_max_len)
    #   相关帧队列
    queue_frame = []
    queue_contours = []
    #   视频处理
    while cap.isOpened():
        ...
        #   前景检测
        contours_KNN = model_process(gray_frame, knn, noise_thres, max_area)

        #   光照影响消减
        if len(queue_contours) < queue_max_len or queue_max_len <= 1:
            contours_better = contours_KNN
            #print(frame_count, 'meijinle')
        else:
            #print(frame_count,'jinle')
